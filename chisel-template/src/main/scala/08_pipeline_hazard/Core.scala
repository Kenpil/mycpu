...
class Core extends Module {
  ...
  // 各種データ系
  //val exRs1Addr = RegInit(0.U(WORD_LEN.W)) 
  //val maRs1Addr = RegInit(0.U(WORD_LEN.W)) 
  //val wbRs1Addr = RegInit(0.U(WORD_LEN.W)) 
  //val exRs1Data = RegInit(0.U(WORD_LEN.W)) 
  //val maRs1Data = RegInit(0.U(WORD_LEN.W)) 
  //val wbRs1Data = RegInit(0.U(WORD_LEN.W)) 
  //val exRs2Addr = RegInit(0.U(WORD_LEN.W)) 
  //val maRs2Addr = RegInit(0.U(WORD_LEN.W)) 
  //val wbRs2Addr = RegInit(0.U(WORD_LEN.W)) 
  ...
  // アドレスが非ゼロならデフォではレジスタから読み取り
  // MA，WBステージでrs1/rs2レジスタが書き換わるならフォワーディング
  // val rs1Data = Mux(rs1Addr =/= 0.U(WORD_LEN.W), regFile(rs1Addr), 0.U(WORD_LEN.W))
  val rs1Data = MuxCase(regFile(rs1Addr), Seq(
    (rs1Addr === 0.U(WORD_LEN.W)) -> 0.U(WORD_LEN.W), // 0なら何もせん
    ((rs1Addr === maRdAddr) && ((maWbP === RD_W) || (maWbP === JMP_W) || (maWbP === CSR_W))) -> maWbData, // MAがrs1レジスタ書き換え命令ならフォワーディング
    ((rs1Addr === wbRdAddr) && ((wbWbP === RD_W) || (wbWbP === JMP_W) || (wbWbP === CSR_W))) -> wbWbData, // WBがrs1レジスタ書き換え命令ならフォワーディング
  ))
  //val rs2Data = Mux(rs2Addr =/= 0.U(WORD_LEN.W), regFile(rs2Addr), 0.U(WORD_LEN.W))
  val rs2Data = MuxCase(regFile(rs2Addr), Seq(
    (rs2Addr === 0.U(WORD_LEN.W)) -> 0.U(WORD_LEN.W), // 0なら何もせん
    ((rs2Addr === maRdAddr) && ((maWbP === RD_W) || (maWbP === JMP_W) || (maWbP === CSR_W))) -> maWbData, // MAがrs2レジスタ書き換え命令ならフォワーディング
    ((rs2Addr === wbRdAddr) && ((wbWbP === RD_W) || (wbWbP === JMP_W) || (wbWbP === CSR_W))) -> wbWbData, // WBがrs2レジスタ書き換え命令ならフォワーディング
  ))
  // I形式(LWとか)の即値を抜き出す
  ...
  // ID/EXデータハザード対策
  // データハザードのストールを検出したらFlgを1にする
  val idExStallFlg = Mux(
    // ID命令の書き込み先が非ゼロ かつ
    // ID命令とEX命令の書き込み先が同じ かつ
    // EX命令がレジスタ書き込み命令 の時ストール
    (((rs1Addr =/= 0.U(WORD_LEN.W)) && (rs1Addr === exRdAddr) && ((exWbP === RD_W) || (exWbP === JMP_W) || (exWbP === CSR_W))) || // rs1のハザードがあるか，
    ((rs2Addr =/= 0.U(WORD_LEN.W)) && (rs2Addr === exRdAddr) && ((exWbP === RD_W) || (exWbP === JMP_W) || (exWbP === CSR_W)))), // rs2のハザードがあれば，
    true.B, false.B) // ストールフラグをtrueにする
  ...
  //val BubbleFcn = List(ALU_ADD, 0.U(WORD_LEN.W), 0.U(WORD_LEN.W), ALU_R, RD_W)
  //val exeFcn::op1Data::op2Data::readP::wbP::Nil = Mux(idExStallFlg, BubbleFcn, cSignals)
  //val exeFcn::op1Data::op2Data::readP::wbP::Nil = Mux(idExStallFlg, 
  //  Array(), cSignals)
  //val exeFcn::op1Data::op2Data::readP::wbP::Nil = BubbleFcn
  ...
  //exReadP := readP
  //exWbP := wbP
  exExeFcn := Mux(idExStallFlg, ALU_ADD, exeFcn)
  exRdAddr := Mux(idExStallFlg, 0.U(5.U), rdAddr)
  exOp1Data := Mux(idExStallFlg, 0.U(WORD_LEN.W), op1Data)
  exOp2Data := Mux(idExStallFlg, 0.U(WORD_LEN.W), op2Data)
  exReadP := Mux(idExStallFlg, ALU_R, readP)
  exWbP := Mux(idExStallFlg, RD_W, wbP)
  // デコード値系
  exImm_i := imm_i
  ...
  // ストールフラグがtrueならID/EXストール対策
  when(idExStallFlg){
    pc := pcOrg // pcは今のママ
    // パイプラインレジスタも今のママ
    idPcReg := idPcReg
    idPcPlus4Reg := idPcPlus4Reg
    idInstReg := idInstReg
    // 現在のIDの内容をバブル化する
    //exeFcn := ALU_ADD
    //rdAddr := 0.U(5.U)
    //op1Data := 0.U(WORD_LEN.W)
    //op2Data := 0.U(WORD_LEN.W)
    //readP := ALU_R
    //wbP := RD_W
    //exExeFcn := ALU_ADD
    //rdAddr := 0.U(5.U)
    //exOp1Data := 0.U(WORD_LEN.W)
  ...
  io.exit := (maPcReg === 0x44.U(WORD_LEN.W))
  ...
  printf(p" -rs1Addr : 0x${Hexadecimal(rs1Addr)}\n")
  printf(p"  rs2Addr : 0x${Hexadecimal(rs2Addr)}\n")
  printf(p"  rdAddr : 0x${Hexadecimal(rdAddr)}\n")
  printf(p"  exRdAddr : 0x${Hexadecimal(exRdAddr)}\n")
  printf(p"  exWbP : 0x${Hexadecimal(exWbP)}\n")
  printf(p"maPcReg: 0x${Hexadecimal(maPcReg)}\n")
  printf(p"  io.dmem.wData: 0x${Hexadecimal(io.dmem.wData)}\n")
  //printf(p"  maRs1Data : 0x${Hexadecimal(maRs1Data)}\n")
  //printf(p"  maRs2Data : 0x${Hexadecimal(maRs2Data)}\n")
  printf(p"wbPcReg: 0x${Hexadecimal(wbPcReg)}\n")
  printf(p"  regFile(0x${Hexadecimal(wbRdAddr)}): 0x${Hexadecimal(regFile(wbRdAddr))}\n")
  printf(p"  wbWbData : 0x${Hexadecimal(wbWbData)}\n")
  //printf(p"  wbRs1Data : 0x${Hexadecimal(wbRs1Data)}\n")
  //printf(p"  wbRs2Data : 0x${Hexadecimal(wbRs2Data)}\n")
  printf(p"idExStallFlg : ${idExStallFlg}\n")
