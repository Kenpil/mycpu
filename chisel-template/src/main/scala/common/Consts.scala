...
object Consts {
  val WORD_LEN      = 32
  val START_ADDR    = 0.U(WORD_LEN.W) // 32bit幅
  ...
  val EXE_FUN_LEN = 5
  val ALU_X       =  0.U(EXE_FUN_LEN.W) // 5bit幅
  ...
  val CSR_RS      = 18.U(EXE_FUN_LEN.W)
  val CSR_RC      = 19.U(EXE_FUN_LEN.W)
  val ALU_COPY1   = 20.U(EXE_FUN_LEN.W)
  val ALU_VADDVV  = 21.U(EXE_FUN_LEN.W)
  val VSET        = 22.U(EXE_FUN_LEN.W)
  val ALU_PCNT    = 23.U(EXE_FUN_LEN.W)

  /*val OP1_LEN = 2
  val OP1_RS1 = 0.U(OP1_LEN.W) // 2bit幅
