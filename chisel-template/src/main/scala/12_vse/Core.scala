...
class Core extends Module {
  ...
  // VADD用データ
  val vsewAdd = csrRegFile(VTYPE_ADDR)(4, 2)
  val vlmulAdd = csrRegFile(VTYPE_ADDR)(1, 0)
  val lmulAdd = (1.U << vlmulAdd) // lmul = 2 ^ vlmul このlmulの個数分のレジスタを使う
  val vs1Data = Cat(
    Seq.tabulate(8)(n => Mux(n.U < lmulAdd, vecRegFile(rs1Addr + n.U), 0.U(VLEN.U))).reverse
  )
  val vs2Data = Cat(
    Seq.tabulate(8)(n => Mux(n.U < lmulAdd, vecRegFile(rs2Addr + n.U), 0.U(VLEN.U))).reverse
  )
  val vRdData = Cat(
    Seq.tabulate(8)(n => Mux(n.U < lmulAdd, vecRegFile(rdAddr + n.U), 0.U(VLEN.U))).reverse
  )
  ...
  val cSignals = ListLookup(io.imem.inst,
    ...
    Array(
      ...
      VLE -> List(ALU_ADD, rs1Data, 0.U(WORD_LEN.W), VMEM_R, VRD_W),
      // VADD用に渡すもののbit幅が不定なためここではデータは渡さない
      VADDVV -> List(ALU_VADDVV, 0.U(WORD_LEN.W), 0.U(WORD_LEN.W), ALU_VR, VRD_W),
      VSE -> List(ALU_ADD, rs1Data, 0.U(WORD_LEN.W), ALU_VR, VMEM_W),
    )
  )
  ...
  val sewCsr = (1.U(1.W) << (3.U(3.W) + vsewCsr)).asUInt() // VSEW 000 -> 32bit みたいにSEW化

  // あとで値変えるのでWireDefault型
  val vaddvv = WireDefault(0.U((VLEN * 8).W))
  //val vNum = VLEN * 8 / sewCsr // LMUL=8(MAX)時の要素数
  for(vsew <- 0 to 7){
    val sewCandi = 1 << (vsew + 3)
    val vNum = VLEN * 8 / sewCandi
    when(sewCsr === sewCandi.U){
      vaddvv := Cat(Seq.tabulate(vNum)
        (
          n => (vs1Data(sewCandi * (n + 1) - 1, sewCandi * n) 
            +  vs2Data(sewCandi * (n + 1) - 1, sewCandi * n))
        ).reverse
      )
    }
  }
  ...
  //}
  io.dmem.wEn := Mux(wbP === MEM_W, true.B, false.B)
  //io.dmem.vWEn := false.B
  //when(wbP === VMEM_W){
  //  io.dmem.wWEn := true.B // SWならメモリへの書き込みenableをtrue
  //}
  val loadLen = sewCsr * vlCsr // 実際にロードしたいbit幅
  io.dmem.vWEn := Mux(wbP === VMEM_W, true.B, false.B)
  io.dmem.vWLen := loadLen
  io.dmem.vWData := vRdData
  // CSR系
  ...
  // ベクトルレジスタへのWB．VADDVV命令なら加算結果，普通のVLEならdmemの値
  val wbVData = Mux(readP === ALU_VR, vaddvv, io.dmem.vData)
  ...
  when(wbP === VRD_W){
    ...
    //printf(p"vlmul: ${vlmul}, lmul: ${lmul}\n")
    ...
    for(rep <- 0 to 7){
      when(rep.U < repVLENMax){
        //printf(p"    vvvvvvvv! rep: ${rep}\n")
        // repVLENMax未満ならVLENbit数だけ全部レジスタに突っ込む
        vecRegFile(rdAddr + rep.U) := wbVData(VLEN * (rep + 1) - 1, VLEN * rep) // .Uつける型がダメ
      }.elsewhen(rep.U === repVLENMax){
        //printf(p"    vvvvvvvvLast! rep: ${rep}\n")
        // 余りを含む最後の回
        val tailLen = VLEN.U - remainder // そっとしておくtailの個数
        val vecMemOrg = wbVData(VLEN * (rep + 1) - 1, VLEN * rep)
  ...
  printf(p"vs1Data : 0x${Hexadecimal(vs1Data)}\n")
  printf(p"vs2Data : 0x${Hexadecimal(vs2Data)}\n")
  printf(p"vaddvv : 0x${Hexadecimal(vaddvv)}\n")
