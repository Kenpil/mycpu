...
class dMemory extends Bundle{
    ...
    val vWEn = Input(Bool())
    val vWLen = Input(UInt(WORD_LEN.W))
...
class Memory extends Module {
    ...
    loadMemoryFromFile(mem, "src/hex/vse32_m2.hex") // ctest用
    ...
    // vWEn信号がhighならメモリに書き込み
    when(io.dmem.vWEn === true.B){
        //print("vWEn!\n")
        // 8bitデータ区切りなので，まずは書き込みたい8bit要素数を数える
        val vWNum = io.dmem.vWLen / 8.U
        for(rep <- 0 to (VLEN - 1)){ // LMUL=8で最大VLEN個の要素がある
          when(rep.U < vWNum){
              mem(io.dmem.addr + rep.U) := io.dmem.vWData((rep + 1) * 8 - 1, rep * 8)
          }
        }
    }
